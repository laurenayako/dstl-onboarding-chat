...
def llm_title(content):
    response = client.chat.completions.create(
        model="gemma3",
        messages=[
            {"role": "user", "content": f"Generate a short title based on this message: {content}. Do not add any filler words or anything. Only print the singular, best title in your opinion"}
        ]
    )
